...
- 文档状态：`OWNER_APPROVED`
- 编制日期：2026-08-01
- Owner 批准日期：2026-08-01
- 权威玩法输入：`docs/P02_GAMEPLAY_BASELINE.md` v1.1
...
- 当前初始夹具 18 名高一、4 名高二可支撑三年生命周期，但包含 2★及旧字段，不能当 P02 正式夹具。
...
```text
{ status: VALID, setup: MatchSetup }
| { status: NEEDS_REPAIR, lastSubmittedSetup: MatchSetup }
```

`VALID` 分支必须相对当前 active roster 完整合法；年级推进/毕业使其失效时，在同一 lifecycle settlement 中改成 `NEEDS_REPAIR`，保留最后一次玩家提交快照以便生成建议，但 repair proposal 本身即时派生、不持久化。`SET_MATCH_SETUP` 只接受一份完整合法快照并把状态恢复为 `VALID`。`SCRIMMAGE` 在 `NEEDS_REPAIR` 下仍可推进，因为它忽略默认 12 人和首发；`OFFICIAL/FRIENDLY` 固定返回 `MATCH_SETUP_INVALID`，直到玩家或 Gate 自动政策显式提交修复。
...
| `SET_MATCH_SETUP`                                | 原子保存 12 人、首发位置、职责、三轴战术和助教预设 |                                  否 |
| `COMPLETE_OPERATION_WEEK` / `COMPLETE_EXAM_WEEK` | 完成对应阶段并提交一次全局事务                     | 是，仅运营周固定派生一次 match seed |

`SET_MATCH_SETUP` 接受完整快照，不允许先提交“12 人已改、首发仍引用旧人”的暂时非法中间态。毕业导致默认名单失效时：

1. 状态允许保存“需要修复”的 setup 状态；
2. 系统生成确定性的 `AUTO_REPAIR_PROPOSAL`，只作为建议；
3. 正式/友谊赛开始前玩家必须提交一份合法完整 setup；
4. 快速自动政策和三年 Gate 可显式接受该建议并提交；
5. 不在玩家不知情时静默改写默认名单。

这样既不隐藏硬锁，也不违背“系统不能直接覆盖玩家设置”。
...
1. 校验 phase、计划完整性、正式赛优先和总承诺成本；正式/友谊赛还必须校验完整合法 setup，队内赛忽略可能已失效的默认 12 人/首发，只校验活跃人数并使用确定性蓝白分组；
...
本切片同时冻结三年 Gate 自动政策 fixture：`policyId = p02-gate-auto-v1`。canonical policy 文件必须包含并 hash：每个运营周采用标准训练、七个项目按运营周序号循环、无球队活动和无专项；开放周保留免费队内赛；合法 setup 跨周沿用，失效时显式接受确定性 repair proposal；所有比赛选择 `INSTANT`，战术和助教轮换使用平衡预设。政策不得读取比赛胜负后临时优化，任何修改都提升 policy ID，并使不同 policy 的候选不可直接比较。
...
- 毕业导致默认 setup 失效后，队内赛仍可按活跃名单完成；直到下一场正式/友谊赛才以 `MATCH_SETUP_INVALID` 拒绝，提交修复 setup 后可继续；
...
| 最早阶段 | 内容                                                                                      | P02 允许保留                           |
| -------- | ----------------------------------------------------------------------------------------- | -------------------------------------- |
| P03      | 招募、新生、年度补员、可持续 roster、正式星级生成                                         | 固定 3★/4★夹具                         |
