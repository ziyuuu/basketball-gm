...
- 文档状态：`OWNER_APPROVED`
- 编制日期：2026-08-01
- Owner 批准日期：2026-08-01；12 人球队修订：2026-08-02；单场体力与强制错位修订：2026-08-06；P02-003 Gate 范围纠偏：2026-08-07
- 权威玩法输入：`docs/P02_GAMEPLAY_BASELINE.md` v1.4
...
## 0.5 v1.3 单场体力与强制错位修订

Owner 于 2026-08-06 冻结
`docs/design/P02-003/P02_SINGLE_MATCH_ENERGY_AND_FORCED_MISMATCH_AMENDMENT.md`。P02-003 在进入
B8 前增加一次定点合同修复：不重开 P02-002 Schema、时钟、Event/Fact、RNG 或 replay，
但必须把 Model B 的开场状态、单场消耗、恢复、体力档位和位置兜底改为该修订的唯一语义。
旧的“赛前疲劳 10 vs 80”和“玩家主动五人错配”不再是 Gate 场景。

## 0.6 v1.4 P02-003 Gate 范围纠偏

Owner 于 2026-08-07 冻结
`docs/design/P02-003/P02_003_SCOPE_CORRECTION_AND_GATE_B_DOWNGRADE.md`。P02-003 只以 Headless
Model B 的可观察最小闭环验收；逐行为 target 精确归属、替换双层原因和产品级位置恢复/
防振荡策略降级为非阻断或延期到 P02-006。现有 `internal/test` 中性轮换只需合法、确定、
可终止，不得被提升为产品助教规则。
...
| 当前占位                                   | 不能继续沿用的原因                     | 替代                             |
| ------------------------------------------ | -------------------------------------- | -------------------------------- |
| 四项 `offense/defense/athleticism/stamina` | 不符合获批 11 能力模型                 | GameState V2 球员能力            |
...
- 单场体力按已提交时间片累计：场上时间基础消耗与实际行为参与者追加消耗分开记账，
  Anchor 兼容字段保存本场已消耗体力，首个 Anchor 全员为 `0`；
- 替补席、普通节间、中场、加时与暂停恢复边界符合 v1.3 定点修订；
- 正常阵容只使用主位置；只有对应位置无人可用时才允许确定性强制错位；
...
2026-08-07 Gate 纠偏后，中性轮换的精确阈值、主位置恢复时点、保护期、双层解释原因，
以及 non-selectable 行为的唯一 target 均不属于 P02-003 阻断。P02-003 只要求三类比赛可
终止、阵容合法、犯满者不返场、无人同位置可补时能确定性继续，并保持 replay 一致。
...
调参前先提交不可变的场景 registry。每项必须登记 64 个固定配对 seed、唯一被改变的输入维度、公式、分母、无候选分母时的处理、平局处理、配对阈值和汇总阈值；修改 registry 即提升场景版本并重新跑全部候选，不能只保留有利 seed。四个正式基线场景和一个增补场景如下：
...
| 高耐力 vs 低耐力续航     | 双方都从 `100` 单场体力开场，只把受测一方全员 `stamina` 从预注册低值改为高值；同时间、同实际行为负荷下，高耐力方终场和第四节开始时平均剩余体力更高、第四节平均体力档位惩罚更低，三项汇总方向均成立且各至少 `40/64` 配对同方向。不要求高耐力提高恢复速度或必然提高胜场。                                                                                    |
...
| 浅轮换 vs 深轮换         | 在 P02-006 产品助教策略完成后加入：深轮换第四节结束时主动轮换池平均剩余体力更高；浅轮换上半场首发五人分钟占全队球员分钟比例更高；两项汇总方向均成立，且各至少 `40/64` 配对同方向。                                                                                                                                                                         |
| 弱队适配战术不抹平实力差 | 作为第五个增补场景：同一强弱直接对阵中只把弱队改为 registry 预声明的最适配战术，强队平均分差仍 `> 0`，弱队最多 `31/64` 获胜；它不替代上述轮换场景。                                                                                                                                                                                                        |

主动五人错配场景删除，不以另一种主动错位夹具替代。强制错位在 P02-003 只验证：
犯满者离场且不返场、有对应主位置候选时优先补位、对应位置无人可用时确定性触发一次
统一能力惩罚并继续比赛。主位置恢复和正式自动轮换语义由 P02-006 验收。P02-003 先通过明显强弱队、高低耐力
续航、战术适配和“弱队适配不抹平差距”；P02-006 再补浅/深轮换。所有指标必须由
Anchor、结构化事件或事件 reducer 得出。以上是 P02 工程方向 Gate，不是最终一年或三年
胜率承诺。
...
- 11 能力、每项 XP/软上限、不可训练身高/臂展、6 倾向、0～1 特质、周训练状态和个人默契；单场体力不持久化进球员成长状态；
...
- 登记顺序、首发、职责和轮换的确定性推荐；
- 主位置覆盖、强制错位原因/统一扣减、角色摘要、即时加权场上默契；
- 三轴战术和 effect 映射；
- 浅/平衡/深助教轮换；
- 完整执教无主动轮换标记；
- 强制离场补位；
...
- 推荐可生成但不静默提交；
- 换人后职责补派和默契即时重算；
- 三种助教剩余体力阈值、最短轮换段、体力差和主动池；
- 64 个配对 seed 中，深轮换在第四节结束时主动轮换池平均剩余体力至少高 6 点，浅轮换上半场首发五人分钟占全队球员分钟比例更高；两项各至少 40/64 配对方向一致；不要求深轮换胜率更高；
- 完整执教无主动换人仍能完成；
- 犯满者不可返场；
- 蓝白分组稳定、人数和位置/能力尽量均衡；
- 正常主位置补位、无人可补时的强制错位、不同默契和三轴收益/代价方向；不得建立玩家主动错位夹具。
...
| Match contract          | Anchor、event、replay、体力与强制错位   | `domain/match`                            |
...
| Directional             | 强弱、耐力续航、战术、轮换              | `tests/p02-directional-scenarios.test.ts` |
...
| P08      | 29 属性必要性、完整倾向/徽章、士气/专注/压力必要性、深度统计和内容深度平衡                | 11 能力、6 倾向、6 单级特质、周状态/单场体力/默契 |
...
- [x] Owner 于 2026-08-06 批准单场体力与强制错位修订并将本文更新为 v1.3；
