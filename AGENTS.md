...
- The Owner-approved P02 gameplay authority is `docs/P02_GAMEPLAY_BASELINE.md` v1.4, as narrowly amended for P02-003 and downstream player-model work by `docs/design/P02-003/v2.9-r1-final/P02_GAMEPLAY_BASELINE_AMENDMENT.md`, for single-match energy plus forced position mismatch by `docs/design/P02-003/P02_SINGLE_MATCH_ENERGY_AND_FORCED_MISMATCH_AMENDMENT.md`, and for P02-003 Gate scope by `docs/design/P02-003/P02_003_SCOPE_CORRECTION_AND_GATE_B_DOWNGRADE.md`. The scope-correction decision wins only for P02-003 acceptance granularity and blocker classification; it does not erase the downstream product direction. For P02-003, versioned files under `docs/design/P02-003/v2.9-r1-final/errata/` take precedence over the exact clauses they correct unless a later Owner decision explicitly replaces them. Do not reinterpret the authority from superseded vision or audit drafts.
- The Owner-approved implementation authority is `docs/P02_DEVELOPMENT_PLAN.md` v1.4. Implement it through one unblocked GitHub Issue and one independently reviewable PR at a time.
...
- P02 single-match energy always starts at 100 and is independent from pre-match training state. Time creates base consumption, actual behavior participants create additional consumption, and stamina only slows consumption. Below 80 energy, all match abilities except stamina lose 5 points per 10-energy band, capped at -30; immutable height/wingspan do not change. Players cannot choose an off-position lineup: deterministic forced mismatch is allowed only when no primary-position player is available. The legacy `secondaryPosition` field has no product effect.
